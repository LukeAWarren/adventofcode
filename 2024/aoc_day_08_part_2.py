...
def find_antenna_distances(antenna_locations, antinode_locations_map):
    ...
    for first_index in range(num_locations - 1):
        ...
        for second_index in range(first_index + 1, num_locations):
            ...
            # the two pairs are always antinode locations
            antinode_locations_map[first_x][first_y] = '#'
            antinode_locations_map[second_x][second_y] = '#'
            dist_x = second_x - first_x
            dist_y = second_y - first_y
            print(f"dist_x: {dist_x}, dist_y: {dist_y}")
            # set all antinodes from first antenna
            first_antinode_location_x = first_x - dist_x
            first_antinode_location_y = first_y - dist_y
            while (first_antinode_location_x >= 0 and first_antinode_location_x < map_size and
                first_antinode_location_y >= 0 and first_antinode_location_y < map_size):
                print(f"first_antinode_location = {first_antinode_location_x},{first_antinode_location_y}")
                antinode_locations_map[first_antinode_location_x][first_antinode_location_y] = '#'
                first_antinode_location_x = first_antinode_location_x - dist_x
                first_antinode_location_y = first_antinode_location_y - dist_y

            second_antinode_location_x = second_x + dist_x
            second_antinode_location_y = second_y + dist_y
            while (second_antinode_location_x >= 0 and second_antinode_location_x < map_size and
                ...
                second_antinode_location_x = second_antinode_location_x + dist_x
                second_antinode_location_y = second_antinode_location_y + dist_y

origainal_antenna_locations_map = load_data('aoc_day_08_part_2.dat')
